describe XlibObj::Event::ClientMessage do
  ...
  describe '#send_to: Sending a message to a window' do
    ...
    let(:client_event) { {} }
    let(:event_mask) { Xlib::SubstructureNotifyMask | Xlib::
      SubstructureRedirectMask }

    before { allow(Xlib::XEvent).to receive(:new).and_return(event_union) }
    before { allow(event_union).to receive(:[]).with(:xclient).and_return(
      client_event) }
    before { allow(XlibObj::Atom).to receive(:new).with(display, :message_type).
      and_return(double(to_native: :message_type_atom_id)) }
    ...
    it { is_expected.to send_message(:[]=).to(client_event).with(:message_type,
      :message_type_atom_id) }
    it { is_expected.to send_message(:XSendEvent).to(Xlib).with(:display_ptr,
      :receiver_win_id, false, event_mask, :event_ptr) }
    ...
    context "when a subject is given" do
      before { instance.subject = instance_double(XlibObj::Window, to_native:
        :subject_win_id) }
      it { is_expected.to send_message(:[]=).to(client_event).with(:window,
        :subject_win_id) }
    end
    ...
    context "when no data is given" do
      ...
      it { is_expected.not_to send_message(:[]=).to(client_event).with(:format,
        anything) }
    end

    context "when data is given" do
      let(:sent_data) { { l: [], s: [], b: [] } }
      before { allow(client_event).to receive(:[]).with(:data).and_return(
        sent_data) }

      context "as 5 longs" do
        let(:data) { [1,2,3,4,5] }
        before { instance.data = data }
        it { is_expected.to send_message(:[]=).to(client_event).with(:format,
          32) }
